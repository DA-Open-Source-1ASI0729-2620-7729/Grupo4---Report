...
| :--- | :--- | :--- | :--- | :--- |
| US01 | Autenticación | **Como** usuario de RoadWatch, **quiero** poder iniciar sesión **para** acceder a mi cuenta y perfil | **Dado** que el usuario ingresa credenciales, **cuando** son válidas, **entonces** se redirige al dashboard. | EP08: Usuario |
...
| US06 | Caso de Uso / Testimonio | **Como** usuario de RoadWatch **quiero** ver un caso de éxito o cifra de impacto **para** confiar en la efectividad de la plataforma. | **Dado** que el usuario hace scroll en la landing, **cuando** llega a la sección de impacto, **entonces** ve métricas ilustrativas. | EP07: Reporte |
| US07 | Notificaciones Push Landing | **Como** usuario de RoadWatch **quiero** aceptar notificaciones del navegador **para** recibir alertas de incumplimientos sin ingresar a la plataforma. | **Dado** que el usuario entra al sitio, **cuando** aparece el prompt de permiso, **entonces** el sistema registra la suscripción push. | EP03: Alerta |
...
| US16 | Feed de Comentarios en Incidencia | **Como** usuario de RoadWatch **quiero** ver el historial de comentarios de una incidencia **para** entender el contexto de su seguimiento. | **Dado** que hay mensajes previos, **cuando** se abre la incidencia, **entonces** se visualizan en formato de burbujas de texto con fecha y autor. | EP04: Incidencia |
| US17 | Alertas y Notificaciones | **Como** usuario de RoadWatch **quiero** un icono de notificaciones **para** ver alertas de incumplimientos recientes. | **Dado** que llega una nueva alerta, **cuando** el usuario ve el header, **entonces** la campana muestra un punto rojo con el conteo. | EP03: Alerta |
...
| US19 | Selector de Rango de Fechas | **Como** usuario de RoadWatch, **quiero** elegir un rango de fechas **para** revisar el histórico de mediciones de un periodo específico. | **Dado** que se abre el selector, **cuando** se elige un rango, **entonces** el dashboard filtra los datos a ese periodo. | EP06: Dashboard |
...
| US21 | Login Minimalista | **Como** usuario de RoadWatch **quiero** una interfaz de login minimalista **para** reducir la fricción al entrar a la plataforma. | **Dado** que el usuario carga la URL, **cuando** no hay sesión activa, **entonces** ve un login centrado y limpio. | EP08: Usuario |
...
|---|---|---|---|---|
| 1 | US21 | Login Minimalista | Como usuario de RoadWatch, quiero una interfaz de login minimalista para reducir la fricción al entrar a la plataforma. | 2 |
...
| 4 | US01 | Autenticación | Como usuario de RoadWatch, quiero poder iniciar sesión para acceder a mi cuenta y perfil. | 2 |
...
| 15 | US10 | Tarjetas de Proyecto (Cards) | Como usuario de RoadWatch, quiero ver tarjetas visuales de cada proyecto para entender su estado ambiental de un vistazo. | 3 |
...
| 21 | US19 | Selector de Rango de Fechas | Como usuario de RoadWatch, quiero elegir un rango de fechas para revisar el histórico de mediciones de un periodo específico. | 3 |
...
| 27 | US17 | Alertas y Notificaciones | Como usuario de RoadWatch, quiero un icono de notificaciones para ver alertas de incumplimientos recientes. | 2 |
...
| 31 | US16 | Feed de Comentarios en Incidencia | Como usuario de RoadWatch, quiero ver el historial de comentarios de una incidencia para entender el contexto de su seguimiento. | 3 |
...
| 48 | US06 | Caso de Uso / Testimonio (Landing) | Como usuario de RoadWatch, quiero ver un caso de éxito o cifra de impacto para confiar en la efectividad de la plataforma. | 2 |
...
| 55 | US07 | Notificaciones Push Landing | Como usuario de RoadWatch, quiero aceptar notificaciones del navegador para recibir alertas de incumplimientos sin ingresar a la plataforma. | 5 |
